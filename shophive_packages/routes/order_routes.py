...
def create_order():
    ...
    # Add order items
    for item in items:
        order_item = OrderItem(order_id=order.id, product_id=item['product_id'], quantity=item['quantity'], price=item['price'], seller_id=1)
        db.session.add(order_item)
    db.session.commit()
    
    return jsonify({
        "status": "success",
        "data": {"order_id": order.id, "total_amount": order.total_amount, "status": order.status},
...
@app.route('/api/orders/<int:order_id>', methods=['PATCH'], strict_slashes=False)
...
@app.route('/api/orders/<int:order_id>/status', methods=['GET'], strict_slashes=False)
